...
router.post("/registration", upload.single("photoImageName"), (request, response) => {
  ...
  const statement = `INSERT INTO StudentInfo
       ( email, password, courseId, address, photoImageName, birthDate, gender, roleId) 
       VALUES ( ?, ?, ?, ?, ?, ?, ?, ?)`;

       connection.query(
    statement,
    [ email, password, courseId, address, photoImageName, birthDate, gender,2], (error, result) =>  {
      console.log(stdName);
