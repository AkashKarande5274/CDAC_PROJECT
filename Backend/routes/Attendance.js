...
router.get('/attendance/:stdId', (req, res) => {
    const stdId = req.params.stdId; // Extract student ID from URL
    const sql = `SELECT * FROM studentattendence WHERE stdId = ?`; // SQL Query with placeholder
    ...
    connection.connect((connectError) => {
        ...
        connection.query(sql, [stdId], (queryError, result) => {
            if (queryError) {
                ...
                res.status(404).json({
                    status: 'error',
                    message: `No attendance details found for student ID: ${stdId}`
                });
            } else {
                res.status(200).json({
                    status: 'success',
